...
import { Package, Clock, TrendingUp, Scan, Pause, Box, Coffee, LogOut, AlertTriangle, CheckCircle, Play, MapPin } from 'lucide-react';
import { User, Session, ConsumableRequest, HelpRequest } from '../types';
import { HOLD_REASONS, CONSUMABLE_TYPES } from '../constants';
import { todayStr, fmtDur, uid } from '../utils';
import { saveSessions, getSessions, saveRequests, getRequests, getStation, getShowConsumables, getEnabledConsumables, getConsumableTypes, getStock, saveStock, getHelpRequests, saveHelpRequests, getTargetOrdersPerHour, getLeaderboard } from '../services/dataService';
import { normalizeScan, validateBinFormat, validateOrderFormat, detectSla, isDuplicateOrder, DEFAULT_BIN_REGEX } from '../lib/validation';
...
export const PackerStation: React.FC<PackerStationProps> = ({ user, onLogout, t, lang, setLang }) => {
  ...
  const enabledConsumables = getEnabledConsumables(consumableTypes);
  const [sessions, setSessions] = useState<Session[]>(() => getSessions().filter(s => s.packer_id === user.uid && s.date === todayStr()));
  const [activeSession, setActiveSession] = useState<Session | null>(() => getSessions().find(s => s.packer_id === user.uid && s.status === 'IN_PROGRESS') || null);
  ...
  const [isConsumOpen, setIsConsumOpen] = useState(false);
  const [selectedConsum, setSelectedConsum] = useState('BOX_MEDIUM');
  const [consumToast, setConsumToast] = useState(false);
  const [helpToast, setHelpToast] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);
  const [showCancelConfirm, setShowCancelConfirm] = useState(false);
  const [lastPacked, setLastPacked] = useState<{ binId: string; orderId: string; duration: string } | null>(null);
  ...
  const targetOrdersHr = getTargetOrdersPerHour();
  const performancePercent = Math.min(Math.round((ordersHr / targetOrdersHr) * 100), 100);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      setBreakSec(Math.max(0, 3600 - elapsed));
    }, 500);
    return () => clearInterval(iv);
  }, [isOnBreak, breakStart]);

  useEffect(() => {
    if (!isHoldOpen && !isConsumOpen && !showSummary && !showSuccess && !isOnBreak)
      setTimeout(() => inputRef.current?.focus(), 100);
  }, [isHoldOpen, isConsumOpen, showSummary, showSuccess, isOnBreak, activeSession]);

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Simulate physical button with 'H' key (Help)
  ...
  const persist = (all: Session[]) => {
    const others = getSessions().filter(s => !(s.packer_id === user.uid && s.date === today));
    saveSessions([...others, ...all]);
    setSessions(all);
  };
  ...
  const handleScan = (e: React.FormEvent) => {
    ...
    // Normalize: uppercase + strip control chars + multi-scan guard
    ...
    if (!activeSession) {
      // ── FIRST SCAN: bin ID ──────────────────────────────────────────────
      if (!validateBinFormat(val, DEFAULT_BIN_REGEX)) {
        setError(`Invalid bin format: "${val}"`);
        return;
      }
      const session: Session = {
        id: uid(), packer_id: user.uid, packer_name: user.name, station,
        bin_id: val, start_time: Date.now(), status: 'IN_PROGRESS',
        date: today, holds: []
      };
      ...
      const updated: Session = {
        ...activeSession, status: 'COMPLETED', order_id: val,
        end_time: Date.now(), cycle_time_seconds: dur
      };
      const all = sessions.map(s => s.id === updated.id ? updated : s);
      persist(all);
      setActiveSession(null);
      setLastPacked({ binId: activeSession.bin_id, orderId: val, duration: fmtDur(dur) });
      setShowSuccess(true);
  ...
  const handleHold = () => {
    ...
    const all = sessions.map(s => s.id === updated.id ? updated : s);
    persist(all); setActiveSession(updated); setIsHoldOpen(false);
  };
  ...
  const handleResume = () => {
    ...
    const updated: Session = { ...activeSession, status: 'IN_PROGRESS', holds };
    const all = sessions.map(s => s.id === updated.id ? updated : s);
    persist(all); setActiveSession(updated);
  };

  const handleCancel = () => {
    if (!activeSession) return;
    const updated: Session = { ...activeSession, status: 'CANCELLED', end_time: Date.now() };
    const all = sessions.map(s => s.id === updated.id ? updated : s);
    persist(all); setActiveSession(null);
    setShowCancelConfirm(false);
  };

  const handleConsumableRequest = (type: string) => {
    const req: ConsumableRequest = {
      ...
      requested_at: Date.now(), date: today,
      status: 'PENDING'
    };
    ...
    const newStock = { ...currentStock, [type]: Math.max(0, (currentStock[type] || 0) - 1) };
    saveStock(newStock);
  ...
  const handleCallSupervisor = () => {
    const currentStation = getStation() || 'UNKNOWN';
    ...
    // Don't spam if already pending for this station
    if (all.some(r => r.station === currentStation && r.status === 'PENDING')) {
      setHelpToast(true);
      setTimeout(() => setHelpToast(false), 3000);
      return;
    }
    
    saveHelpRequests([...all, req]);
    setHelpToast(true);
    setTimeout(() => setHelpToast(false), 3000);
  };
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={s.label} className="bg-white rounded-xl p-3 border border-gray-100 shadow-sm text-center">
              ...
              {s.target && (
                <div className="text-[8px] font-bold text-gray-400 uppercase mt-1">Target: {s.target}/h</div>
              )}
            </div>
          ))}
        </div>
...
<div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
  ...
  <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
    <div 
      className={`h-full transition-all duration-1000 ${totalOrders >= targetOrdersHr ? 'bg-green-500' : 'bg-blue-500'}`}
      style={{ width: `${Math.min(Math.round((totalOrders / targetOrdersHr) * 100), 100)}%` }}
    />
  </div>
  {totalOrders >= targetOrdersHr ? (
    <div className="mt-2 text-[10px] text-green-600 font-bold text-center animate-bounce">
      Target Achieved! Keep it up! 🚀
    </div>
  ) : (
    <div className="mt-2 text-[10px] text-orange-500 font-bold text-center">
      {targetOrdersHr - totalOrders} more orders to hit target
    </div>
  )}
</div>

        {/* Leaderboard for Packer */}
            ...
            {isOnHold && (
              <div className="flex gap-2 mt-3">
                <button onClick={handleResume}
                  className="flex-1 py-2 bg-green-600 text-white text-sm font-semibold rounded-xl hover:bg-green-700 transition-colors flex items-center justify-center gap-1">
                  <Play size={14} />
                  {t.resumePacking}
                </button>
                <button onClick={() => setShowCancelConfirm(true)}
                  className="py-2 px-4 bg-white text-red-500 border border-red-200 text-sm font-semibold rounded-xl hover:bg-red-50 transition-colors">
                  {t.cancelOrder}
                </button>
              </div>
            )}
        ...
        {showConsumables && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {consumableTypes.filter(ct => enabledConsumables.includes(ct)).map(ct => (
              ...
              <button 
                ...
                disabled={isOnBreak}
                className="px-3 py-3 rounded-xl border-2 border-red-100 bg-red-50 hover:bg-red-100 hover:border-red-300 text-xs font-bold text-red-700 transition-all flex flex-col items-center gap-1 active:scale-95 disabled:opacity-40 col-span-2 sm:col-span-1"
              >
                <div className="w-2 h-2 rounded-full bg-red-600 animate-pulse mb-1"></div>
                {t.callSupervisor}
              </button>
            </div>
        ...
        {!isOnBreak && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
            ...
            <form onSubmit={handleScan} className="flex gap-2">
              ...
              <button type="submit" disabled={isOnHold || !inputVal.trim()}
        ...
        {!activeSession && !isOnBreak && (
          <p className="text-center text-xs text-gray-400">{t.step1}</p>
        )}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 pb-8 flex gap-3 shadow-[0_-10px_20px_rgba(0,0,0,0.05)] z-40">
        {activeSession && !isOnHold && (
          <button onClick={() => setIsHoldOpen(true)}
            ...
            <Pause size={24} />
            <span className="text-xs uppercase tracking-wider">{t.holdPause}</span>
          </button>
        )}

        <button onClick={handleBreak}
          disabled={!!activeSession}
          className={`flex-1 flex flex-col items-center justify-center gap-1 h-20 rounded-2xl font-bold transition-all active:scale-95 ${isOnBreak ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' :
        ...
        <button onClick={handleEndShift}
          ...
          className={`flex-1 flex flex-col items-center justify-center gap-1 h-20 rounded-2xl font-bold transition-all active:scale-95 ${isOnBreak || activeSession ? 'bg-gray-100 text-gray-400 cursor-not-allowed' :
              'bg-gray-900 text-white shadow-lg shadow-gray-300'
            }`}>
          <LogOut size={24} />
          <span className="text-xs uppercase tracking-wider">{t.endShift}</span>
        </button>
      </div>
      ...
      {consumToast && (
        <div className="fixed top-5 left-1/2 -translate-x-1/2 z-50 bg-green-600 text-white px-6 py-3 rounded-full shadow-xl flex items-center gap-2 animate-in slide-in-from-top duration-300">
          <CheckCircle size={15} />
          <span className="text-sm font-semibold">{t.consumableRequested}</span>
        </div>
      )}
      ...
      {showSuccess && lastPacked && (
        <div className="fixed inset-0 bg-emerald-500/90 z-50 flex items-center justify-center animate-in fade-in duration-200">
          <div className="bg-white p-10 rounded-3xl shadow-2xl text-center animate-in zoom-in duration-300 max-w-xs w-full mx-4">
            ...
            <div className="text-gray-500 text-base mb-6">{lastPacked.duration}</div>
      ...
      {isHoldOpen && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm shadow-2xl animate-in zoom-in duration-300">
            ...
            <div className="flex gap-3">
              <button onClick={() => setIsHoldOpen(false)}
                className="flex-1 py-2 text-gray-600 font-medium hover:bg-gray-50 rounded-xl text-sm">{t.cancel}</button>
              <button onClick={handleHold}
                className="flex-1 py-2 bg-orange-600 text-white font-semibold rounded-xl hover:bg-orange-700 text-sm">{t.confirmHold}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed top-5 left-1/2 -translate-x-1/2 z-50 bg-red-600 text-white px-6 py-3 rounded-full shadow-xl flex items-center gap-2 animate-in slide-in-from-top duration-300">
          ...
          <span className="text-sm font-semibold">{t.helpRequested}</span>
        </div>
      )}
            ...
            <div className="bg-white p-6 rounded-2xl w-full max-w-sm shadow-2xl animate-in zoom-in duration-300">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleCancel}
                  className="flex-1 py-2 bg-red-600 text-white font-semibold rounded-xl hover:bg-red-700 text-sm">{t.approve}</button>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="bg-white p-6 rounded-2xl w-full max-w-sm shadow-2xl animate-in zoom-in duration-300">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={confirmEndShift}
                  className="flex-1 py-2 bg-gray-900 text-white font-semibold rounded-xl hover:bg-gray-800 text-sm">{t.endShift}</button>
              </div>
            </div>
          </div>
        )}
