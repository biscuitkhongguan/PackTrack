export const SCAN_VALIDATION = {
  /** Default bin regex: accepts any reasonable bin/tote ID */
  DEFAULT_BIN_REGEX: "^[A-Z0-9][A-Z0-9_\\-]{1,48}$",

  /** Order regex: 8-50 chars, alphanumeric + hyphen + underscore
   *  Supports standard formats AND instant/sameday (e.g. 3M_SO69A91CD6C9E77C000131F2AA-1) */
  ORDER_REGEX: /^[A-Z0-9][A-Z0-9_\-]{6,48}[A-Z0-9]$/,
};

/**
 * Normalizes a raw scan value:
 * - Trims & uppercases
 * - Strips invisible control characters
 * - Multi-Scan Guard: if scanner accidentally double/triple-fires the same code, collapse it
 */
export function normalizeScan(input: string): string {
  if (!input) return "";
  let clean = input.trim().toUpperCase().replace(/[\x00-\x1F\x7F]/g, "");

  // Detect repeated scans of same code (common scanner hardware bug)
  for (let n = 4; n >= 2; n--) {
    if (clean.length % n === 0) {
      const chunkLen = clean.length / n;
      if (chunkLen > 3) {
        const chunk = clean.substring(0, chunkLen);
        let isRepeated = true;
        for (let i = 1; i < n; i++) {
          if (clean.substring(i * chunkLen, (i + 1) * chunkLen) !== chunk) {
            isRepeated = false;
            break;
          }
        }
        if (isRepeated) {
          console.log(`[Multi-Scan Guard] Detected ${n}x repeat: "${clean}" → "${chunk}"`);
          return chunk;
        }
      }
    }
  }
  return clean;
}

/** Validates a bin/tote ID against a configurable regex (from settings) */
export function validateBinFormat(input: string, customRegex?: string): boolean {
  try {
    const regex = new RegExp(customRegex || SCAN_VALIDATION.DEFAULT_BIN_REGEX);
    return regex.test(input);
  } catch {
    return false;
  }
}

/** Validates an order label format */
export function validateOrderFormat(input: string): boolean {
  return SCAN_VALIDATION.ORDER_REGEX.test(input);
}

/**
 * Determines SLA tier from an order ID:
 * - length > 25 chars → Instant / Same-day order
 * - otherwise → Regular
 */
export function detectSla(orderId: string): 'Regular' | 'Instant' {
  return orderId.length > 25 ? 'Instant' : 'Regular';
}
